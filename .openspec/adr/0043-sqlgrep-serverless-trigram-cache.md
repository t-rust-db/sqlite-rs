...
1. **`sqlgrep` is a `[[bin]]` in this crate**, `src/bin/sqlgrep/`, behind a
   `sqlgrep` cargo feature that is on by default (like `cli`). It depends on
...
2. **One SQLite-format cache file per canonicalized root**, at
   `$SQLGREP_CACHE_DIR` or `dirs::cache_dir()/sqlgrep/<fnv1a64(root)>.db`.
...
3. **Incremental update is one `Pager` transaction per invocation.** Files
   ...
   *adds* the trigrams it did not have; lost trigrams and deleted files stay
   ...
   Crash safety is `Pager::open`'s hot-journal recovery; sqlgrep adds no
   bookkeeping.
...
6. **ADR-0040 decision 2 is amended:** a *binary-only, feature-gated*
   ...
   are declared optional under the `sqlgrep` feature; `default-features =
...
- **An in-crate `.gitignore` parser or the `ignore` crate.** The former is a
  ...
  exactly.
- **Putting sqlgrep in its own repository.** It is an application over
