...
//! `sqlgrep` crash safety (#34): `kill -9` the indexer at many points,
...
//! Recovery is `Pager::open`'s hot-journal rollback; sqlgrep adds no
...
use sqlite_rs::btree::{count_table_rows, TableCursor};
use sqlite_rs::dump;
...
fn scratch() -> (PathBuf, PathBuf) {
    let dir = std::env::temp_dir().join(format!("sqlite-rs-sqlgrep-crash-{}", std::process::id()));
    ...
    // lists stay similar in size — see `sqlgrep_cli_test.rs::
