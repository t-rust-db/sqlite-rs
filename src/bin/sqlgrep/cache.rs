...
use sqlite_rs::record::{encode_record, Value};
...
pub const CACHE_DIR_ENV: &str = "SQLGREP_CACHE_DIR";
...
pub fn cache_path(root: &Path) -> Result<PathBuf> {
    let dir = match std::env::var_os(CACHE_DIR_ENV) {
        ...
        None => dirs::cache_dir()
            .ok_or("no cache directory for this platform; set SQLGREP_CACHE_DIR")?
            .join("sqlgrep"),
    };
...
fn read_roots(pager: &Pager, header: &DatabaseHeader) -> Result<Option<(u32, u32)>> {
    ...
    let root_of = |name: &str| -> Result<u32> {
        schemas
            ...
            .find(|s| s.name == name)
            .map(|s| s.root_page)
            .ok_or_else(|| format!("not a sqlgrep cache: table {name:?} missing").into())
    };
    root_of("meta")?;
